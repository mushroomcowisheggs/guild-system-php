...
### 现有透明性措施
- **公共账本 API**（`GET /api/submit.php`）：同样返回脱敏的猎人列表。
- **SQLite 文件可下载**：如果服务器配置允许，将 `data/guild.db` 文件设为公开可读（注意需脱敏或加密邮箱字段），任何人都可以下载原始数据库，用 SQLite 工具本地查询验证数据一致性。
- **开源代码**：代码完全公开，用户可以审查事务逻辑、唯一约束等。

### 透明性缺陷
| 问题 | 说明 |
|------|------|
| **实际操作仍依赖服务器诚信** | 虽然数据库文件可下载，但下载的快照可能是过时的或被篡改后再提供。除非服务器持续提供**实时哈希签名**（例如每隔 N 分钟将数据库哈希发布到第三方不可篡改公告板），否则用户无法确认当前 API 返回的数据是否与服务器内部一致。 |
| **无操作审计日志** | 与 DO 版相同，`admin.php` 直接 `UPDATE hunters SET status`，没有独立的审计表记录操作历史。管理员可以不留痕迹地篡改。 |
| **单方控制修改权** | 同样依赖 `ADMIN_SECRET`，拥有者完全控制。 |
| **SQLite 文件若公开会泄露邮箱** | 若想公开数据库文件以验证完整性，邮箱字段会暴露隐私。需要额外设计（如邮箱哈希化）。 |

**结论**：虽然 SQLite 文件的可下载性比 DO 版略好一点（理论上用户可获取完整数据快照），但**仍然缺乏防篡改、可审计、去中心化的核心机制**。本质上仍是中心化信任。未来需要引入更多措施。

### 待实现功能
| 功能 | PHP + SQLite 版 |
|------|------------------|
| **防刷（速率限制）** | 使用 SQLite 临时表 `rate_limits`，定期清理过期记录 |
| **多签核销** | 相同逻辑，从 `$_SERVER` 读取自定义头 |
| **审计日志** | 新建 `audit_log` 表，`INSERT` 后 `COMMIT` |
| **Merkle 树公开验证** | 在 `common.php` 中增加函数 `getMerkleRoot($hunters)`，GET 响应头中携带 |
| **定期对外哈希** | 使用操作系统的 `cron` + `curl` 调用本地 PHP 脚本，或者使用远程监控服务（如 UptimeRobot） |
